...
import { flattenTrilha, stopPath, findStopIndex } from '../lib/deck.js'
...
export default function DeckView() {
  ...
  const base = `/${curso}/${trilhaSlug}`
  const stops = trilha ? flattenTrilha(trilha) : []
  const total = stops.length
  const idx = trilha ? findStopIndex(stops, aula, slide) : -1

  function goTo(i) {
    if (i < 0 || i >= total) return
    navigate(stopPath(base, stops[i]))
  }
